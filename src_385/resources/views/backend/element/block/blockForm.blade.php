        ...
        <div class="card" style="padding:10px">
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <input name="{{ $block->input_name }}[]" value="{{ !empty($landingPageItemsData) ? $landingPageItemsData[$block->input_name][$i]:'' }}" class="form-control" type="text" placeholder="" />
                </div>
                ...
                @elseif($block->input_type == 'select2')
                    <label>{{ $block->name or '' }}</label>
                    @php
                        $categorys = app('EntityCommon')->getRowsByConditions('category', [], 0, ['sort_order', 'asc'], json_decode($block->conditions, true));
                    @endphp
                    <select class="form-control" name="{{ $block->input_name }}">
                        @foreach($categorys as $cate)
                        <option {{ (!empty($landingPageItemsData[$block->input_name][$i]) && $cate->id == $landingPageItemsData[$block->input_name][$i]) ? 'selected="selected"':'' }} value="{{ $cate->id }}">{{ $cate->name }}</option>
                        @endforeach
                    </select>
