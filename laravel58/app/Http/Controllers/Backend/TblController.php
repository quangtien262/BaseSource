...
class TblController extends BackendController
{
    ...
    public function postSbmitFormTable(Request $request, $tableId = 0)
    {
        ...
        try {
            ...
            //generate model
            if(!app('UtilsCommon')->checkExistModel($request->input('model_name'))) {
                app('UtilsCommon')->createModel($request->input('table_name') ,$request->input('model_name'));
            }
    ...
    public function deleteTable(Request $request)
    {
        if (!empty($request->input('table'))) {
            ...
            if (!empty($table)) {
                ...
                //delete in tbl config
                app('ClassTables')->deleteTable($request->input('table'));
                app('EntityCommon')->deletesTable(TBL_COLUMN, ['table_id' => $table->id]);
            }
        }
